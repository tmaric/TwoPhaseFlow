...
//	scalar iter = 0;
//   TODO: average p2 with a time window
/*        FIFOStack<scalar> windowTimes;
        FIFOStack<volScalarField> windowPr;
	FIFOStack<volTensorField> windowMomFlux;
	scalar window = (2/f).value();*/ 
	if(runTime.timeIndex() > runTime.startTimeIndex() + 1)
	{
	    prAver == mesh.lookupObject<volScalarField>("prMean");
            momFluxAver == mesh.lookupObject<volTensorField>("momFluxMean");
	}
