...
Description
    ...
    2x2 block Helmholtz system with a custom tensor-laplacian assembly
    for the PML coupling term B1:

        [ A  -(B1 + B2) ] [Pim] = [bPim]
        [ (B1 + B2)  A ] [Pre]   [bPre]

    where A is the main Helmholtz operator and B1/B2 are PML-induced
    coupling operators. B1 is assembled directly into PETSc as
    div(T1.grad(p)), avoiding the scalar fvm::laplacian(TC1, p)
    approximation. The PETSc solve uses a direct backend
    (default: MUMPS via LU).
...
int main(int argc, char *argv[])
{
    ...
    if (Pstream::parRun())
    {
        FatalErrorInFunction
            << "tensorLaplacianFoam uses serial assembly. "
    ...
    while (simple.loop())
    {
        ...
        while (simple.correctNonOrthogonal())
        {
            ...
            // Pim-row couples to Pre via (div(T1.grad(Pre)) + Sp(SC1,Pre))
            fvScalarMatrix couplingMassPre(fvm::Sp(SC1, Pre));

            // Pre-row couples to Pim via (div(T1.grad(Pim)) + Sp(SC1,Pim))
            fvScalarMatrix couplingMassPim(fvm::Sp(SC1, Pim));

            assembleTensorBlockSystem
            (
                M, globalCells, N,
                AopPim, AopPre,
                couplingMassPre, couplingMassPim,
                T1, Pim, Pre
            );
            ...
            scalarField bTensorPre(tensorLaplacianBoundarySource(T1, Pre));
            bPim -= bTensorPre;
            ...
            scalarField bTensorPim(tensorLaplacianBoundarySource(T1, Pim));
            bPre += bTensorPim;
