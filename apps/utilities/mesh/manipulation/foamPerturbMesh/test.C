...
int main(int argc, char* argv[])
{
    ...
    fvMesh mesh(IOobject("region0",
        "constant",// runTime.constant(), //"constant",
        "constant",//"system",//runTime.constant(),
        runTime,
        IOobject::MUST_READ,
        IOobject::AUTO_WRITE,
        false));
    Info << "mesh points: " << mesh.nPoints() << endl;
    const auto alpha = args.getOrDefault<scalar>("alpha", 0.1);

    auto pertubationVecPtr = randomUnitVec(mesh.points().size());
    Info << "pertubationVecPtr " << pertubationVecPtr.ref()[10000] << pertubationVecPtr.ref()[15000] << pertubationVecPtr.ref()[20000] << pertubationVecPtr.ref()[24697] << endl;
    ...
    Info << "tpertubations " << tpertubations.ref()[10000] << tpertubations.ref()[15000] << tpertubations.ref()[20000] << tpertubations.ref()[24697] << endl;
    ...
    Info << "mesh.points " << mesh.points()[10000] << mesh.points()[15000] << mesh.points()[20000] << mesh.points()[24697] << endl;
