...
def plot_dframe(dFrame, dFrameAgglomerator, case_path,study_name ,title="", plotDict = {}, ncol=2):
    ...
    ax.legend(loc='upper center', borderaxespad=0, bbox_to_anchor=(0.5, -0.15),fancybox=True, shadow=True, ncol=ncol) 
#    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5),prop={'size':7}, fancybox=True, shadow=True, ncol=ncol)

    split_n= case_path.split('/')
    split_study_n= study_name.split('.')
#    fig_name='Trans'+'_'+split_study_n[0]+'_'+split_n[0]+'_'+split_n[1]+'_'+split_n[2]
#    plt.savefig('figures/%s.pdf'% fig_name, dpi=400,bbox_inches='tight')
#    figures_path = os.environ['LENT_DENSITY_RATIO']
  #  plt.savefig(os.path.join(figures_path,"%s.png"% fig_name))
#    plt.savefig('figures/%s.png'% fig_name)

    plt.show()

#    dFrame.to_csv(r'./csv_data/%s.csv'% fig_name, encoding='utf-8', index=False )
