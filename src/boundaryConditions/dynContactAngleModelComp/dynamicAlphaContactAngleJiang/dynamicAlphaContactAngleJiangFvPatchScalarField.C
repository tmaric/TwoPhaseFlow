...
) const
{
    ...
    const volScalarField& nu1 = this->db().objectRegistry::lookupObject<volScalarField>("nu1");

    const dictionary& transportProperties = this->db().objectRegistry::lookupObject<IOdictionary>
    (
        "transportProperties"
    );
    ...
    {
        Info << "found   contactAngle" << endl;
        volScalarField &contactAngle=const_cast<volScalarField&>(
        this->db().objectRegistry::lookupObject<volScalarField>("contactAngle"));

        contactAngle.boundaryFieldRef()[patchi] = min(180/constant::mathematical::pi*acos(cos(theta0_)-tanh(scalar(4.96)*pow(pos(Ca)*Ca,0.706))*(cos(theta0_)+1)),scalar(180));
    }
    ...
    return min(180/constant::mathematical::pi*acos(cos(theta0_)-tanh(scalar(4.96)*pow(pos(Ca)*Ca,0.706))*(cos(theta0_)+1)),scalar(180));
}
