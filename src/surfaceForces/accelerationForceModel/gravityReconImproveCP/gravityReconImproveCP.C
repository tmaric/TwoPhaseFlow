...
#include "gravityReconImproveCP.H"
...
namespace Foam
{
    defineTypeNameAndDebug(gravityReconImproveCP, 0);
    addToRunTimeSelectionTable(accelerationForceModel,gravityReconImproveCP, components);
}

Foam::vector Foam::gravityReconImproveCP::closestDist(const point p, const vector n ,const vector centre)
{
    if (mag(n) != 0)
    {
...
Foam::gravityReconImproveCP::gravityReconImproveCP
(
...
void Foam::gravityReconImproveCP::calculateAcc()
{
...
Foam::tmp<Foam::surfaceScalarField> Foam::gravityReconImproveCP::accelerationForce()
{
    const fvMesh& mesh = acc_.mesh();
