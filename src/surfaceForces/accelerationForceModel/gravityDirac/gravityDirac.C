   ...
   //  Info << "### mesh.faces().size(): " << mesh.faces().size() << endl; 
    ...
    if(mesh.foundObject<reconstructionSchemes>("reconstructionScheme"))
    {
        ...
        forAll(cellFaceNormalbf, pi)
        {
            ...
            {
                cellFaceNormalbf[pi] = faceNormal.boundaryField()[pi].patchInternalField(); //(luis) This line was before: cellFaceNormalbf[pi] = faceNormal.boundaryField().boundaryInternalField()[pi];
